jest.mock('@/lib/prisma', () => ({
  ...
  default: {
    ...
    teamCoachAssignment: { findFirst: jest.fn() },
...
const p = prisma as unknown as {
  ...
  teamCoachAssignment: { findFirst: jest.Mock };
...
function source(overrides: Record<string, unknown> = {}) {
  return {
    ...
    coverageFrom: new Date('2026-08-01T00:00:00.000Z'),
    coverageTo: new Date('2027-05-31T00:00:00.000Z'),
    verifiedAt: new Date('2026-09-13T09:00:00.000Z'),
    noteHe: null,
    ...overrides,
  };
}
...
function arrange(year = 2026) {
  p.season.findUnique.mockResolvedValue({ id: `season-${year}`, year, name: `${year}/${String(year + 1).slice(-2)}` });
  ...
  p.player.findMany.mockResolvedValue([
    {
      ...
      jerseyNumber: 8,
      playerStats: [
        { appearances: 2, gamesPlayed: 2, starts: 2, minutesPlayed: 180, goals: 1, assists: 1, position: 'Midfielder' },
        { appearances: null, gamesPlayed: 1, starts: 1, minutesPlayed: 90, goals: 2, assists: 0, position: 'Midfielder' },
      ],
    },
    {
      id: 'player-2',
  ...
  p.teamCoachAssignment.findFirst.mockResolvedValue({
    coachNameHe: 'רן מאמן',
...
describe('buildSeasonDossier', () => {
  ...
  it('hides draft editorial, moments, and moment sources from public payloads', async () => {
    ...
    expect(payload?.sources.map((row) => row.id)).toEqual(['source-metrics', 'source-editorial']);
  ...
  it('uses assignment identity and API-Football image when no canonical coach is linked', async () => {
    p.teamCoachAssignment.findFirst.mockResolvedValueOnce({
      coachId: null,
