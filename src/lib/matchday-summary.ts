...
export function buildMatchdaySummary(games: AttendedGame[]) {
  const summary = { attended: games.length, completed: 0, hbsGames: 0, wins: 0, draws: 0, losses: 0 };
  for (const game of games) {
    if (game.status !== 'COMPLETED' || !Number.isInteger(game.homeScore) || !Number.isInteger(game.awayScore)) continue;
    summary.completed++;
