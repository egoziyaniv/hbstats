...
type Question = { id: string; opponent: string; season: string; date: string; home: boolean };
export default function QuizQuestion({ question }: { question: Question }) {
  ...
  return <section className="space-y-5 rounded-2xl border bg-white p-6"><h2 className="text-xl font-bold">איך הסתיים המשחק של באר שבע מול {question.opponent}?</h2><p className="text-sm text-stone-600">לפי התוצאה השמורה, ללא הכרעת בעיטות פנדלים.</p><p>עונת {question.season} · {question.home ? 'בבית' : 'בחוץ'} · {new Date(question.date).toLocaleDateString('he-IL', { timeZone: 'Asia/Jerusalem' })}</p><div className="flex flex-wrap gap-3">{['ניצחון', 'תיקו', 'הפסד'].map(choice => <button key={choice} disabled={busy || !!answer} onClick={() => submit(choice)} className="rounded-xl border border-stone-300 px-5 py-3 font-bold disabled:opacity-50">{choice}</button>)}</div><p role="status">{error || (busy ? 'בודקים…' : '')}</p>{answer && <div className="space-y-4" aria-live="polite"><p className="text-xl font-bold">{answer.correct ? 'נכון!' : 'הפעם לא.'} {answer.fact.result} — {answer.fact.goalsFor} שערי זכות, {answer.fact.goalsAgainst} שערי חובה.</p><a href={answer.fact.source} className="underline">מקור התשובה: דף המשחק ב־StatsAI</a><MatchFactShare fact={answer.fact} /></div>}</section>;
}
